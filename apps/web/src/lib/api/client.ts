...
export const api = {
  ...
  getRun: (id: UUID) => apiRequest<AgentRunDetail>(`/api/v1/agent-runs/${id}`),
  getEvents: (id: UUID, afterSequence = 0, limit = 100) => apiRequest<EventList>(`/api/v1/agent-runs/${id}/events`, { query: { after_sequence: afterSequence, limit } }),
  retryRun: (id: UUID, key: string) => apiRequest<RunAccepted>(`/api/v1/agent-runs/${id}/retry`, { method: "POST", idempotencyKey: key }),
